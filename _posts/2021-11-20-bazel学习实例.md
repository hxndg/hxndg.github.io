...
我们有两套buildfarm服务，每个都是三个server，六个worker，最近通过看metric:execulte_slog_usage发现buildfarm1的负载很重，需要启用两套环境，因此必须保证同一个ip的所有请求必须打到一个server上，要么bf1要么bf2，否则会出现确认文件存在bf2后，开始执行在bf1上，触发异常excetion报错cas找不到precondition。这里要注意，我没启用一致性hash，我认为两套都一直是available的，实际上升级了buildfarm版本并且cp了相关代码之后确实稳定了很多。

经过我的实验，无论是ingress还是直接用service都不太成，service默认是rr，而ingress同一个path只会匹配第一个service，及时配置多个依然无效，所以只能启用nginx来做负载均衡，有三种具体的配置：

+ ip_hash ：这种方法是不生效的，因为这个只会使用ipv4的整个四个位段的前三个，除非我们是ipv6，才会用整个的（The first three octets of the client IPv4 address, or the entire IPv6 address, are used as a hashing key. ）对于局域网环境ip的前三个字段都是一致的，所以是无法正常负载均衡的，和这个链接https://stackoverflow.com/questions/69481839/nginx-ip-hash-balancing-method-not-working-as-expected 写的一样，这种方法排除。

+ hash $remote_addr consistent：这个会启用[ketama](https://www.metabrew.com/article/libketama-consistent-hashing-algo-memcached-clients)一致性算法，这个算法保证及时添加或者删除服务器，也只有少数的连接被重新映射。但是这个算法严格意义来说是不符合我们的要求的：我们是多个nginx pod，这些nginx pod在对bf1 bf2的service做hash的时候会有随机的digest（Nmmm这是未初始化导致的问题）来hash出来不同的整数，也就是说他们内部hash出来的结果是不一样的，思考这么一种情况，client第一个check 文件再cas的请求达到了nginx pod 1上，nginx pod 1映射的时候映射到了bf2，返回存在，然后client认为满足了请求，开始远程执行，remote exec请求到了nginx pod 2上，nginx pod 2映射的时候映射到了bf1，执行报错。。。
...
+ hash $remote_addr：这个是正常的结果，从具体的算法来看，就是很直接的hash，没有什么莫名其妙的随机情况。
...
所以最终我们选择了这种配置来做nginx的负载均衡。

```nginx
upstream xxxxx {
    server A;
    server B;
    hash $remote_addr; #需要使用这种方法，而不是下面两种方法，为什么呢
  	# ip_hash;
