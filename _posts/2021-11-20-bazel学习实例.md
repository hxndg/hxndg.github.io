...
codechecker流程，如何安装？直接使用pip3 install codechecker即可

+ `CodeChecker log` runs the given build command and records the executed compilation steps. These steps are written to an output file (Compilation Database) in a JSON format.

+ `CodeChecker analyze` uses the previously created JSON Compilation Database to perform an analysis on the project, outputting analysis results in a machine-readable (plist) format.
...
+ In this step, you can do multiple things:
