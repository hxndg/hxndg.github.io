...
“存储库”是开发人员在其上工作的源代码树；它通常代表一个项目。Bazel 的祖先 Blaze 在 monorepo 上运行，即包含用于运行构建的所有源代码的单个源代码树。相比之下，Bazel 支持源代码跨越多个存储库的项目。调用 Bazel 的存储库称为“主存储库”，其他存储库称为“外部存储库”。

存储库由其根目录中名为`WORKSPACE`（或`WORKSPACE.bazel`）的文件标记。此文件包含对整个构建“全局”的信息，例如，可用的外部存储库集。它像普通的 Starlark 文件一样工作，这意味着可以使用`load()`其他 Starlark 文件。这通常用于拉入显式引用的存储库所需的存储库（我们称之为“`deps.bzl`模式”）

外部存储库的代码在 `$OUTPUT_BASE/external`.

运行构建时，需要将整个源代码树拼凑在一起；这是由 SymlinkForest 完成的，它将主存储库中的每个包`$EXECROOT`和每个外部存储库符号链接 到`$EXECROOT/external`或 `$EXECROOT/..`（当然前者使得`external`在主存储库中调用包是不可能的；这就是我们要从它迁移的原因）

### 套餐

每个存储库都由包组成，即相关文件的集合和依赖项的规范。这些由名为`BUILD`or的文件指定 `BUILD.bazel`。如果两者都存在，Bazel 更喜欢`BUILD.bazel`; BUILD 文件仍然被接受的原因是 Bazel 的祖先 Blaze 使用了这个文件名。然而，事实证明它是一个常用的路径段，尤其是在文件名不区分大小写的 Windows 上。

包是相互独立的：一个包的BUILD文件的改变不会导致其他包的改变。添加或删除 BUILD 文件 _can _change 其他包，因为递归 glob 在包边界处停止，因此 BUILD 文件的存在会停止递归。

BUILD 文件的评估称为“包加载”。它在 class 中实现`PackageFactory`，通过调用 Starlark 解释器工作，并且需要了解可用规则类的集合。包加载的结果是一个`Package`对象。它主要是从字符串（目标名称）到目标本身的映射。

包加载过程中的一大块复杂性是 globbing：Bazel 不需要显式列出每个源文件，而是可以运行 glob（例如`glob(["**/*.java"])`）。与 shell 不同，它支持下降到子目录（但不下降到子包）的递归 glob。这需要访问文件系统，因为这可能会很慢，所以我们实施了各种技巧以使其尽可能高效地并行运行。

Globbing 在以下类中实现：

- `LegacyGlobber`，一个快速而幸福的 Skyframe 不知道的 globber
