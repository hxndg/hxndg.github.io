...
+ 数据库密钥的创建，更新与管理
  ...
  + 用户请求生成AK时
    ...
    + 如果namespace的kek存在，那么检索当前的kek。这个时候使用共享锁锁住这个kek
    + 在锁住kek的前提下，写入ak。总之务必保证，写入ak的时候kek是不变的，这两个是个原子操作。
  
  + 用户申请读取ak时，先获取ak，然后拿这个ak对应的kek号读取kek，之后揭秘kek再解密ak。这个过程不需要任何锁操作。
  + 用户申请更新ak时：读取ak，获得该ak对应kek的版本号，然后读取该版本kek，之后使用这个版本kek加密ak再写入数据库。这个时候要保证写入之前的ak的srand & timestamp & version & kek_version 和之前保持一致。用户希望更新ak的时候，是需要指定旧的版本号的，如果更新的时候发现版本号和请求的版本号不一样，那么就abort这个过程，返回错误。对ak使用独占锁
