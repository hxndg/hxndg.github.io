...
+ 数据库密钥的存储
  ...
  + AK，用户具体使用AK存储各种秘钥
    ...
    这里有一点需要注意，kms agent的私钥也是存储在数据库当中的，
...
+ 数据库密钥的创建，更新与管理
  ...
  + 用户请求生成ak之前，应该先申请生成对应的kek，存储于数据库当中。
    + 单独申请生成对应的kek（感觉这个没必要啊），但是这个可以一定程度简化设计。
    + 要不要每个kek管理的ak，都单独创建一个同名kek的表来管理呢？会麻烦一些。这个我觉得算了，没啥必要

  + 用户请求生成AK时
    + 如果发现namespace对应的kek不存在，那么创建该namespace的kek并锁住这个kek。如果生成kek需要申请，这个自动创建kek就可以去掉
    + 如果namespace的kek存在，那么检索当前的kek。这个时候使用行级别共享锁，锁住kek
    + 在锁住kek的前提下，写入ak。总之务必保证，写入ak的时候kek是不变的，这两个是个原子操作。
  
  + 用户申请读取ak时，先获取ak，然后拿这个ak对应的kek号读取kek，之后揭秘kek再解密ak。过程当中都是行级别共享锁
  + 用户申请更新ak时：读取ak，获得该ak对应kek的版本号，然后读取该版本kek，之后使用这个版本kek加密ak再写入数据库。这个时候要保证写入之前的ak的srand & timestamp & version & kek_version 和之前保持一致。用户希望更新ak的时候，是需要指定旧的版本号的，如果更新的时候发现版本号和请求的版本号不一样，那么就abort这个过程，返回错误。先行级别共享锁拿到kek信息，释放事务。然后新建一个事务，先行级别独占锁锁住kek，再行级别独占锁锁住ak，更新，然后释放事务。更新操作的时候原始数据需要和一开始的一致。
    + 问题1: 读取ak信息获得了kek版本，然后检索kek的时候，发现kek没有的时候，直接失败。需要重新读取ak。获取ak的kek版本号的时候，如果kek还有更新的版本，不用管这个新版本，还是按照上面的步骤来，rotate密钥的流程是单独的rotate线程/协程管理。
  
  + 自动更新kek：先读取旧的kek数据，然后启动一个事务，更新kek先，更新其内容和版本号，然后一条一条更新数据库内使用旧版本的kek加密的ak，整个都成功了，才算是成功。所以更新kek & ak 是原子操作。kek使用独占锁，使得旧版本的kek不会被其他连接读取或者更新，不允许其他连接读取是为了避免读取到旧的kek然后创建新的ak，对kek关联的ak使用独占
