...
+ 这个需求究竟是**客户提出来的，还是只是顺嘴提一句，SE随手加上的**？很多时候SE根本就不懂这个功能，就是胡说八道，因此需要确认是不是真正的需求。
...
2021-05-10 ==> 2021-09-30美团，C++

美团内部的CPP方面的CI/CD实际上构建量很大，很复杂，但是感觉并不够好。确实是缺乏强有力的推动，但是强有力的推动并不该单纯是人的缘故，而应该是基于实际结果触发，复杂的版本构建、包管理、才是根本原因，因此使用的应该是一个类似bazel的moder build system，不该单纯是一个软件库的推广。
...
下午和mike聊了一下进度/问题/总结，聊了一些内容：

+ 从start project的感受和选择入手，有什么问题啥的
+ onboard的感受和问题
+ 对：1环境/工具的感受；2和同伴沟通的感受；3文档的感受
