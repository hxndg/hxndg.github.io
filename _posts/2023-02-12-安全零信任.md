...
+ 什么是安全的证书
  ...
  + 加解密key类型方面：使用安全的加解密秘钥类型，比方说rsa申请4096字节以上
...
+ 如何对证书做自动化管理：
  ...
  + 对于服务pod上的证书，这个服务pod上面的证书更新实际上就是服务pod自己的任务了，agent可以使用mtl访问证书管理服务申请和自己证书san一致的更新证书。或者使用类似acme等方式来进行更新
...
```shell
qcraft@BJ-HeXiaonan:~/qcraft$ ls -l -a  /sys/fs/cgroup/
total 0
...
数据安全能力成熟度模型（Data Security Capability Maturity Mode,简称*DSMM*）是阿里的一套模型，一般来说按照下面的方面进行分类

1. 数据采集安全
   + 数据分级分类机制安全管理，需要能够。**工具方面需要有相应的数据资产管理工具，实现对数据分类分级自动进行标示。**
     + 根据数据不同的级别指定不同的标示和管理
