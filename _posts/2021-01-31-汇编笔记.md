...
=> 0x000055555555517d <+4>:	    sub    $0x30,%rsp
   0x0000555555555181 <+8>:	    mov    %edi,-0x24(%rbp)  #非常标准的对应main函数的两个参数int main(int argc, char** argv),这个是argc
   ...
   0x0000555555555197 <+30>:	cmpl   $0x2,-0x24(%rbp)  #比较有没有参数，换言之有没有输入参数	
   ...
   0x00005555555551c5 <+76>:	movl   $0x0,-0x10(%rbp)     # 清空两个数值
   ...
   0x00005555555551e0 <+103>:	mov    -0xc(%rbp),%eax  #结果是0
   0x00005555555551e3 <+106>:	cltq   
   0x00005555555551e5 <+108>:	add    %rdx,%rax
   0x00005555555551e8 <+111>:	movzbl (%rax),%eax      #取rax，实际上就是输入的数字所在的字符串的1byte，拓展到eax里
   0x00005555555551eb <+114>:	mov    %al,-0x11(%rbp)  #丢到栈里面
   0x00005555555551ee <+117>:	lea    -0x11(%rbp),%rax
   0x00005555555551f2 <+121>:	mov    %rax,%rdi
   0x00005555555551f5 <+124>:	callq  0x555555555060 <atoi@plt> #转换为整数，整个过程是一个循环，就是在不断地将每一位转换为数字，然后加到-0x10(%rbp)上
   0x00005555555551fa <+129>:	add    %eax,-0x10(%rbp)
   0x00005555555551fd <+132>:	addl   $0x1,-0xc(%rbp)  #这里有个很有意思的地方，这里是addl加了一个1，实际上就是诸位比较，还是继续运行，
   0x0000555555555201 <+136>:	mov    -0x30(%rbp),%rax #argv的起始地址，
   0x0000555555555205 <+140>:	add    $0x8,%rax        #偏移8字节，实际上就是找到第二个元素
   0x0000555555555209 <+144>:	mov    (%rax),%rax      #将元素的值赋值给rax，就是输入的数字的所在字符串的地址，
   0x000055555555520c <+147>:	mov    %rax,%rdi
   0x000055555555520f <+150>:	callq  0x555555555040 <strlen@plt>  #对字符串调用strlen判断长度
   0x0000555555555214 <+155>:	cmp    %eax,-0xc(%rbp)              #返回的字符串长度放在了eax里面，和已经操作的比较一下，看看是不是处理完了
   0x0000555555555217 <+158>:	jl     0x5555555551d5 <main+92>
   0x0000555555555219 <+160>:	cmpl   $0x32,-0x10(%rbp)            #比较每一位加起来以后是不是0x32，这就是比较的关键。所以我们构造一个code是0x32求和的即可
