...
流加密虽然理论上需要对不同的segment进行加密，但是看tink的实现就是一个segment进行加密，下面看看流程

tink的流加密分为几个部分，我们先看对一个segment进行加密的部分。可以看到对每个segment进行加密的时候都要初始化aead加解密参数。参考aes_gcm_hkdf_streaming.h文件
